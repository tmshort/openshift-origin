...
var _ = g.Describe("[sig-olmv1] OLMv1 should", func() {
...
var _ = g.Describe("[sig-olmv1] OLMv1 operator installation", func() {
	...
	g.BeforeEach(func() {
		...
		err = oc.AsAdmin().WithoutNamespace().Run("secrets").Args("-n", "openshift-catalogd", "link", "default", "local-ps").Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
	})
	...
	g.AfterEach(func() {
		...
		oc.AdminConfigClient().ConfigV1().ImageDigestMirrorSets().Delete(context.Background(), "catalog-test", metav1.DeleteOptions{})
		...
		oc.AsAdmin().WithoutNamespace().Run("secrets").Args("-n", "openshift-catalogd", "unlink", "default", "local-ps").Execute()
	...
	g.It("should create a catalog", func() {
		...
		err = oc.AsAdmin().WithoutNamespace().Run("policy").
			Args("-n", "openshift-catalogd", "add-role-to-user", "system:image-puller",
				"system:serviceaccount:openshift-catalogd:default").Execute()
		o.Expect(err).NotTo(o.HaveOccurred())

		g.By("creating an IDMS")
		idms := newImageDigestMirrorSet(imageLocation)
...
func newImageDigestMirrorSet(image string) *configv1.ImageDigestMirrorSet {
...
var _ = g.Describe("[sig-olmv1] OLMv1 should have access to certain files", func() {
	...
	g.It("checks the manager containers for those files", func() {
		oc := oc
		pods := []string{
			"catalogd",
			"operator-controller",
